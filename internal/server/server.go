package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-signalman/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-signalman"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/rules",s.handleList)
    s.mux.HandleFunc("POST /api/rules",s.handleCreate)
    s.mux.HandleFunc("POST /api/rules/{id}/fire",s.handleFire)
    s.mux.HandleFunc("POST /api/rules/{id}/toggle",s.handleToggle)
    s.mux.HandleFunc("DELETE /api/rules/{id}",s.handleDelete)
